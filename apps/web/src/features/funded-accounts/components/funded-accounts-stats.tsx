...
export function FundedAccountsStats({ fundedAccounts }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    const byFirmBreached = new Map<string, number>()

    for (const a of fundedAccounts) {
      const name = a.evaluation?.propfirm?.name ?? "Unknown"
      byFirm.set(name, (byFirm.get(name) ?? 0) + 1)

      if (a.status === "active") {
        active++
        byFirmActive.set(name, (byFirmActive.get(name) ?? 0) + 1)
      } else if (a.status === "breached") {
        breached++
        byFirmBreached.set(name, (byFirmBreached.get(name) ?? 0) + 1)
      }
    }
    ...
    return {
      ...
      topBreached: topEntry(byFirmBreached),
      topFirms,
    }
  }, [fundedAccounts])

  const activePct = stats.total ? stats.active / stats.total : 0
  const breachedPct = stats.total ? stats.breached / stats.total : 0
